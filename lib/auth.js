...
import { checkOrigin, clientIp, hasContentType, hasForbiddenKeys, safeNext, warnIfInsecureCookie } from './security'
import { findSessionUser, sessionCookieName } from './session'
...
export const readSessionToken = (req) => req.cookies?.[sessionCookieName()]
...
function createHandler(routes, { requireAuth }) {
  ...
  return async function handler(req, res) {
    ...
    if (STATE_CHANGING.has(req.method)) {
      ...
      if (BODY_METHODS.has(req.method) && !hasContentType(req, contentType)) {
        return res.status(415).json({ success: false, error: `Content-Type must be ${contentType}` })
      }
      if (hasForbiddenKeys(req.body)) {
        logger.warn({ ip, method: req.method, url: req.url }, 'Blocked request body with a prototype key')
...
// have replaced a temporary password. `contentType` overrides the required request
// Content-Type (application/json by default); the file-upload route uses it.
