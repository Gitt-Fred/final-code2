...
// A Secure cookie gets the __Host- prefix, which makes the browser itself refuse it
// unless it is Secure, has Path=/, and has no Domain, so a sibling subdomain or a
// plain-HTTP response can never plant or overwrite it.
export const sessionCookieName = () => (cookieSecure() ? '__Host-crm_session' : 'crm_session')
...
export function buildSessionCookie(token, expiresAt) {
  ...
  return `${sessionCookieName()}=${token}; ${cookieAttributes()}; Max-Age=${maxAge}`
}

export const buildClearedCookie = () => `${sessionCookieName()}=; ${cookieAttributes()}; Max-Age=0`
